...
#include "Ig_Modules/IgWebRPhi/interface/IgWebRPhiService.h"
...
IgWebRPhiService::IgWebRPhiService (IgState *state)
    : IgWebService (state)
{
    registerCallback ("create", lat::CreateCallback (this, &IgWebRPhiService::create));
    registerCallback ("browse", lat::CreateCallback (this, &IgWebRPhiService::browse));
}


void
IgWebRPhiService::create (Arguments *arguments)
{    
...
void
IgWebRPhiService::doCreate (IgState *state, const std::string& name)
{
    ...
    if (browser == 0)
    {
	// FIXME: UGLY hack to get the RPhi view working... 
	LOG (0, trace, LFwebRPhi, 
	     "Creating RPhi browser" << std::endl);
	
	browser = new IgOIVBrowser (state, 0, (Ig3DBaseModel *) -2);
	browser->setCamera ();    
...
void 
IgWebRPhiService::browse (Arguments *arguments)
{
    using namespace lat;
    IgState *state = arguments->state ();
    QIODevice *outputDevice = arguments->outputDevice ();    
...
const char *
IgWebRPhiService::catalogLabel (void)
{
    return "RPhi";
}

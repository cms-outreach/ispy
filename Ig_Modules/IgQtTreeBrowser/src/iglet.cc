...
IG_QT_TREE_BROWSER_API lat::logflag LFqttree = { 0, "qttree", true, -1 };
...
DEFINE_IGUANA_PLUGIN (IgBrowserDB, IgQtTreeBrowser, "Tree");
DEFINE_IG_PLUGIN_NAME (IgCapabilityDB, IgBrowserMethods::key
		       (typeid (IgQtTreeBrowser).name (),
			typeid (IgTwig).name ()));

XTYPEINFO_DEF_1 (IgQtTreeRep, IgRep, false);
XTYPEINFO_DEF_2 (IgQtTreeBrowser, IgBrowser, false, IgModel, false);
...
MMM_DEFUN_FUNC(IgRepContext *,IgBrowserMethods::,doRepresent,
	       (IgTwig *twig, IgQtTreeBrowser *model))
{
    LOG (0, trace, LFqttree, "  doRepresent (IgTwig[" << twig->name ()
	 << "],IgQtTreeBrowser)\n" << lat::indent);
    ...
    IgQtTreeRep		*prep = !ancestor ? 0 : dynamic_cast<IgQtTreeRep *>
				(IgRepSet::lookup (parent, model, true));
    IgQtTreeRep		*prev = 0;
    unsigned		index;

    if (prep && (index = parent->indexOf (twig)) > 0)
	prev = dynamic_cast<IgQtTreeRep *>
	       (IgRepSet::lookup (parent->child (index-1), model));

    IgQtTreeRep *rep = IgQtTreeRep::create (model, prep, prev,
					    twig->name().c_str (),
					    true,
					    twig->selfVisible (),
					    twig->cascade ());
    ...
    LOG (0, trace, LFqttree, lat::undent);
    return new IgRepContext (twig, rep);
}

MMM_DEFUN_FUNC(void,IgBrowserMethods::,doExpand,
	       (IgTwig *twig, IgQtTreeRep *rep))
{
    LOG (0, trace, LFqttree, "  doExpand (IgTwig[" << twig->name ()
	 << "],IgQtTreeRep)\n" << lat::indent);
...
MMM_DEFUN_FUNC(void,IgBrowserMethods::,doCommit,
	       (IgQtTreeRep *rep, IgTwig *twig, unsigned field))
{
    LOG (0, trace, LFqttree, "  doCommit (IgQtTreeRep,IgTwig[" << twig->name ()
	 << "]," << field << ")\n" << lat::indent);
    switch (field)
    {
    case IgQtTreeRep::CASCADE_FIELD:
	// Fiddle the flag; we are the only ones who care about cascade
	twig->cascade (rep->isOpen ());
	break;

    case IgQtTreeRep::TRAVERSE_FIELD:
    ...
    case IgQtTreeRep::VISIBLE_FIELD:
	// Set and propage to other reps
	twig->selfVisible (true);
...
MMM_DEFUN_FUNC(void,IgBrowserMethods::,doUpdate,
	       (IgTwig *twig, IgQtTreeRep *rep, unsigned mask))
{
    LOG (0, trace, LFqttree, "  doUpdate (IgTwig[" << twig->name ()
	 << "],IgQtTreeRep," << mask << ")\n" << lat::indent);

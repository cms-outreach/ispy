window.WindowLego = function (state, parent, id, title)
{
...
WindowLego = window.WindowLego;

importClass ("Studio.Window");

WindowLego.inherits (Window);

WindowLego.method ('init', function (state, parent, id, title)
{
	this.uber ('init', state, parent, id, title, "WindowLego.prototype.onloadCallback (this, '" + id + "')");
    importClass ("Studio.BrowserLego")
	this.browser = new BrowserLego (this, id, 156, 156);
...
WindowLego.method ('onloadCallback', function (placeHolder, id)
{
	placeHolder.setAttribute ("onload", "");
	if (placeHolder == null)
		return;
	var parentDiv = placeHolder.parentNode;
	parentDiv.removeChild (placeHolder);
});

WindowLego.method ('onFocusIn', function (state)
{
...
WindowLego.method ('onFocusOut', function (state)
{
	var toolbar = state.lookup ("Toolbars/Toolbar3D");

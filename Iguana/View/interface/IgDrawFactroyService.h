...
class IgDrawFactroyService : public IgStateElement
{
    IG_DECLARE_STATE_ELEMENT (IgDrawFactroyService);
public:
    typedef SoNode * (*CreateDrawCallback)(IgState *state, const std::string &collection,
					   const std::string &name,
					   const std::string &rep);

    IgDrawFactroyService (IgState *state);
    ~IgDrawFactroyService (void);    

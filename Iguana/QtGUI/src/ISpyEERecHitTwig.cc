...
ISpyEERecHitTwig::ISpyEERecHitTwig (IgState *state, IgTwig *parent, 
					const std::string &name)
    : ISpyQueuedTwig (state, parent, name)
{}

void
ISpyEERecHitTwig::onNewEvent (ISpyEventMessage& message)
{
    ISpyQueuedTwig::onNewEvent (message);

    QSettings settings;    
    QString visSettings ("igtwigs/visibility/");
    visSettings.append ("EERecHits_V1");
	...
	if ( storage->hasCollection("EERecHits_V1") )
	{
		...
		sep->setName (SbName ("ISpyEERecHitTwig"));

...
ISpyTrackingRecHitTwig::ISpyTrackingRecHitTwig (IgState *state, IgTwig *parent,
						const std::string &name /* = "" */)
    : ISpyQueuedTwig (state, parent, name)
{}

void
ISpyTrackingRecHitTwig::onNewEvent (ISpyEventMessage& message)
{
    ISpyQueuedTwig::onNewEvent (message);

    QSettings settings;    
    QString visSettings ("igtwigs/visibility/");
    visSettings.append ("TrackingRecHits_V1");
	...
	{	    
	    ...
	    if (hits.size () > 0 && hits.hasProperty ("pos"))
	    {

...
ISpyCSCSegmentTwig::ISpyCSCSegmentTwig (IgState *state, IgTwig *parent, 
					const std::string &name)
    : ISpyQueuedTwig (state, parent, name)
{}

void
ISpyCSCSegmentTwig::onNewEvent (ISpyEventMessage& message) 
{
    ISpyQueuedTwig::onNewEvent (message);

    QSettings settings;    
    QString visSettings ("igtwigs/visibility/");
    visSettings.append ("CSCSegments_V1");
    ...
    {	
	SoSeparator *rep = dynamic_cast<SoSeparator *>(drawService->draw ("Segments", state (), "CSCSegments_V1", "3D"));

	SoSeparator *sep = dynamic_cast<SoSeparator *>(ISpyQueuedTwig::rep ());
	sep->setName (SbName ("ISpyCSCSegmentTwig"));

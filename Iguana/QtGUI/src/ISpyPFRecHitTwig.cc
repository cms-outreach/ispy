...
ISpyPFRecHitTwig::ISpyPFRecHitTwig (IgState *state, IgTwig *parent, 
				    const std::string &name)
    : ISpyQueuedTwig (state, parent, name)
{}

void
ISpyPFRecHitTwig::onNewEvent (ISpyEventMessage& message) 
{    
    ISpyQueuedTwig::onNewEvent (message);

    QSettings settings;    
    QString visSettings ("igtwigs/visibility/");
    visSettings.append ("PFRecHits_V1");
	...
	if (storage->hasCollection ("PFRecHits_V1"))
	{	    
	    IgCollection &digis = storage->getCollection ("PFRecHits_V1");
		...
		sep->setName (SbName ("PFRecHits_V1"));
